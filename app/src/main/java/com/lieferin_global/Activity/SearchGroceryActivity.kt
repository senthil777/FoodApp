...
class SearchGroceryActivity : AppCompatActivity(),View.OnClickListener,ResponseListener,SearchAdapter.CallbackDataAdapter {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        RequestManager.setUserGroceryActivityDashboard(this, obj, this);
            ...
            if (requestType == Constant.MEMBER_userGroceryDashboard_URL_RQ) {
                ...
                if ((responseObj as BaseRS).status.equals("5801")) {
                    for (i in 0 until (responseObj as BaseRS).fetchData!!.groceryListing!!.size) {
                        ...
                        adapterModel = AdapterModel(R.drawable.img_4, ""+(responseObj as BaseRS).fetchData!!.groceryListing!!.get(i).groceryName, "https://d1zgdcrdir5wgt.cloudfront.net/media/vehicle/images/mVNdVp58T_eVgzLEw-_dpw.730x390.jpg", ""+(responseObj as BaseRS).fetchData!!.groceryListing!!.get(i).street+","+(responseObj as BaseRS).fetchData!!.groceryListing!!.get(i).town, "Order Now", ""+(responseObj as BaseRS).fetchData!!.groceryListing!!.get(i).distance+" KM Away", ""+(responseObj as BaseRS).fetchData!!.groceryListing!!.get(i).rating, ""+(responseObj as BaseRS).fetchData!!.groceryListing!!.get(i).groceryReferenceCode, "4.4", "Very Good", "3054 Ratings>", "5 Star Given by you", "1", "", ""+ (responseObj as BaseRS).groceryPath+"/"+ (responseObj as BaseRS).fetchData!!.groceryListing!!.get(i).folderName, ""+ getImageValue1((responseObj as BaseRS).fetchData!!.groceryListing!!.get(i).groceryImages), "", "", 0, 0, 0,adapterProduct)
                        adapterFeature.add(adapterModel)
                        adapterFeature1.add(adapterModel)
                    }
