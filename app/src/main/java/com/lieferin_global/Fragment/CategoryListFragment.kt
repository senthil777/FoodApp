...
class CategoryListFragment : Fragment(),ResponseListener,FilterSideListPageAdapter.CallbackDataAdapter,View.OnClickListener {
            ...
            if (requestType == Constant.MEMBER_userGroceryDashboard_URL_RQ) {
                if ((responseObj as BaseRS).status.equals("5801")) {
                    ...
                    for (i in 0 until (responseObj as BaseRS).fetchData!!.groceryCategoryList!!.size) {
                        adapterProduct = Product(R.drawable.img_1, ""+(responseObj as BaseRS).fetchData!!.groceryCategoryList!!.get(i).name, ""+(responseObj as BaseRS).fetchData!!.groceryCategoryList!!.get(i).categoryReferenceCode,"","€2.00","1","1","200 for two","","","","","","","","","","",adapterProductList)
                        adapterCategories2.add(adapterProduct)
                    }
