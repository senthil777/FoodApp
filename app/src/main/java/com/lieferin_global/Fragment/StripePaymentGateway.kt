...
import com.lieferin_global.Activity.OrderCompleteActivity
...
import com.lieferin_global.Utility.Constant.customerAddressId
import com.lieferin_global.Utility.Constant.deliveryFare
import com.lieferin_global.Utility.Constant.distanceValue
...
import com.lieferin_global.Activity.AppliyCoupon
...
import kotlinx.android.synthetic.main.offer_cancel.*
import okhttp3.*
...
class StripePaymentGateway : AppCompatActivity(),View.OnClickListener,ResponseListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        backendUrl = Constant.Url+"stripe_pay/create_payment_indent.php?amount="+ addIncreasePrice(Constant.totalPrice.replace("€ ",""),Constant.deliveryFare)+"&receipt_email="+dbHelper!!.getUserDetails().email+"&mode=live"
        ...
        addressData!!.text = addressComplete+""
        ...
        totalData!!.text = "€ "+ DecimalFormat("##.##").format(addIncreasePrice(Constant.totalPrice.replace("€ ",""),deliveryFeeData!!.text.toString()))
        ...
        payButton!!.setOnClickListener {
            if(Constant.PayMentType!!.equals("2")) {
                ...
                if (Constant.totalPrice.replace("€ ", "").toDouble() > 0.4) {
                    ...
                    if (cardNo!!.text.toString().replace(" ", "").length != 16) {
                        ...
                        if (params != null) {
                            backendUrl = Constant.Url+"stripe_pay/create_payment_indent.php?amount="+ totalData!!.text.toString().replace("€ ","")+"&receipt_email="+dbHelper!!.getUserDetails().email+"&mode=live"

                            if(totalData!!.text.toString().replace("€ ","").toDouble() > 0.4) {
    ...
    private fun startCheckout() {
        loadingScreen(this@StripePaymentGateway)
            ...
            .enqueue(object: Callback {
                ...
                override fun onResponse(call: Call, response: Response) {
                    ...
                    if (!response.isSuccessful) {
                        ...
                        loadingScreenClose(this@StripePaymentGateway)
                    } else {
        ...
        //stripe = Stripe(applicationContext, "pk_live_Mg16qTj1TfGm5W7PmqTC98kK00H0CQS7QG")

        stripe = Stripe(applicationContext, "pk_test_cvE7UClVGrrAYLqS0mtFS0HU00NDSTQWuu")
    }
    fun stripValue()
    {
        //val stripe = Stripe(applicationContext, "pk_live_Mg16qTj1TfGm5W7PmqTC98kK00H0CQS7QG")
        val stripe = Stripe(applicationContext, "pk_test_cvE7UClVGrrAYLqS0mtFS0HU00NDSTQWuu")
        val params5 = cardInputWidget!!.paymentMethodCreateParams
        val confirmParams = ConfirmPaymentIntentParams
            .createWithPaymentMethodCreateParams(
                params5!!,
                paymentIntentClientSecret
...
//                                .putExtra("Amount", amount))
                ...
                object : ApiResultCallback<PaymentIntentResult> {
                    override fun onSuccess(result: PaymentIntentResult) {
                        ...
                        if (status == StripeIntent.Status.Succeeded) {
                            ...
                            showToast(this@StripePaymentGateway, "Payment failed")
    ...
    override fun onClick(view: View?) {
        ...
        {
            R.id.back->{
                //createWebServiceFood()
                //callBlacklisting!!.fragmentBack("")
                finish()
            }
            R.id.couponLayout->{
                var intent = Intent(this, AppliyCoupon::class.java)
            ...
            R.id.billCheck->{
                ...
                if(billCheck!!.isChecked)
                {
                    nifNumberCheck = "1"
                    nifNumberLabel!!.visibility = View.VISIBLE

                    if(Constant.nifNumber.equals("")) {
                        nifNumberTV!!.visibility = View.GONE

                        nifNumberET!!.visibility = View.VISIBLE
                    }else{
                        nifNumberTV!!.text = Constant.nifNumber
                        nifNumberTV!!.visibility = View.VISIBLE
            ...
            R.id.billCheckGrocery->{
                if(billCheckGrocery!!.isChecked)
                {
                    nifNumberCheck = "1"
                    nifNumberLabel!!.visibility = View.VISIBLE
            ...
            R.id.cancelImg->{
                ...
                totalData!!.text = "€ "+ DecimalFormat("##.##").format(addIncreasePrice(totalPrice!!.replace("€ ",""),deliveryFeeData!!.text.toString()))
    ...
    fun createWebServiceFood(Payment:String): JSONObject {
        val obj = JSONObject()
        ...
        if(Constant.nifNumber.equals("")) {
            ...
            if(nifNumberCheck.equals("1")) {
                obj.put("nifNumberRequest", "" + nifNumberCheck)
                obj.put("nifNumber", "" + nifNumberET!!.text.toString())
            }else{
                obj.put("nifNumberRequest", "" + nifNumberCheck)
                obj.put("nifNumber", "" )
            }
            ...
            if(nifNumberCheck.equals("1")) {
                obj.put("nifNumberRequest", "" + nifNumberCheck)
                obj.put("nifNumber", "" + nifNumberTV!!.text.toString())
            }else{
                obj.put("nifNumberRequest", "" + nifNumberCheck)
                obj.put("nifNumber", "" )
            }
        ...
        if(!restaurantPromoId!!.equals("")) {
            obj.put("appliedPromocode", "1")
            obj.put("restaurantPromoId", "" + restaurantPromoId)
            obj.put("promoValue", "" + promoValue)
        }
        if(Constant.PickUpType.equals("1"))
        {
            obj.put("bookingDateTime", ""+ PickUpDate)
            obj.put("preBooking", "1")
        }
        ...
        for (i in 0 until dbHelper!!.getMenu().size) {
            ...
            for (j in 0 until dbHelper!!.getMenuGroup(dbHelper!!.getMenu().get(i).id!!).size) {
                ...
                for (k in 0 until dbHelper!!.getMenuGroupToppind(
                    ...
                    if(dbHelper!!.getMenu().get(i).id!!.equals(dbHelper!!.getMenuGroupToppind(dbHelper!!.getMenuGroup(dbHelper!!.getMenu().get(i).id!!).get(j).id!!).get(k).user_id)) {
                        val menusList2 = JSONObject()

                        Log.i("pppppp"+dbHelper!!.getMenu().get(i).id!!+"="+i,"="+dbHelper!!.getMenuGroupToppind(dbHelper!!.getMenuGroup(dbHelper!!.getMenu().get(i).id!!).get(j).id!!).get(k).user_id)
    ...
    fun createWebServiceTable(Payment: String): JSONObject {
        val obj = JSONObject()
        val objArray = JSONArray()
    ...
    fun createWebService(Payment:String): JSONObject {
        val obj = JSONObject()
        val objArray = JSONArray()
        val objArray1 = JSONArray()
        val objArray2 = JSONArray()
        ...
        if(!restaurantPromoId!!.equals("")) {
            obj.put("appliedPromocode", "1")
            obj.put("groceryPromoId", "" + restaurantPromoId)
            obj.put("promoValue", "" + promoValue)
        }
    ...
    override fun onResponseReceived(responseObj: Any?, requestType: Int) {
        if(responseObj != null) {
            ...
            }else if (requestType == Constant.MEMBER_checkGroceryPromoOffer_URL_RQ) {
                showToast(this, (responseObj as BaseRS).message)

                if((responseObj as BaseRS).status.equals("5842"))
                {
                    ...
                    restaurantPromoId  = ""+(responseObj as BaseRS).promoData!!.groceryPromoId
